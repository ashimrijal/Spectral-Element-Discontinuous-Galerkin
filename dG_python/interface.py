def couple_GL(u,v,w,NP,num_elements, dx, rho, mu,x):
    ...
    for j in range(1,num_elements):
        ...
        Hv[(j-1)*NP:NP+(j-1)*NP] = Hv[(j-1)*NP:NP+(j-1)*NP] + 2/(z1s*dx[0]*w[-1])*(p0 - p1)*PH_0
               
        Hu[(j-1)*NP+NP:2*NP+(j-1)*NP] = Hu[(j-1)*NP+NP:2*NP+(j-1)*NP] + 2/(dx[0]*w[-1])*(q1 - q0)*PH_1
        Hv[(j-1)*NP+NP:2*NP+(j-1)*NP] = Hv[(j-1)*NP+NP:2*NP+(j-1)*NP] -2/(z2s*dx[0]*w[-1])*(q1 - q0)*PH_1
...
def couple_friction_GL(u_1,v_1,u_2,v_2,S, psi, w,NP, dx1, dx2, rho1,mu1,rho2,mu2,x,alpha,Tau_0,fric_law):
    ...
    #interface flux penalty functions 
    Hu_1[-NP:] = Hu_1[-NP:] + 2/(dx1[-1]*w[-1])* (p0 - p1)*PH_0
    Hv_1[-NP:] = Hv_1[-NP:] + 2/(z1s*dx1[-1]*w[-1])*(p0 - p1)*PH_0
               
    Hu_2[0:NP] = Hu_2[0:NP] + 2/(dx2[0]*w[0])*(q1 - q0)*PH_1
    Hv_2[0:NP] = Hv_2[0:NP] -2/(z2s*dx2[0]*w[0])*(q1 - q0)*PH_1
